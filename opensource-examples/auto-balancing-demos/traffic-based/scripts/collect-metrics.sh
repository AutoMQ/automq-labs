...
partition_dist=$(docker exec automq-server1-traffic bash -c "
...
if [ "$STAGE" = "before" ]; then
  cat >> "$OUTPUT_FILE" << EOF
This snapshot was taken **before** auto-balancing. The traffic distribution shows:
- Broker 0 is handling the majority of traffic
...
EOF
  ...
  cat >> "$OUTPUT_FILE" << EOF
This snapshot was taken **after** auto-balancing. The traffic distribution shows:
