...
variable "zone" {
  description = "GCP zone for the Console VM and persistent data disk."
  type        = string
...
variable "name_prefix" {
  description = "Prefix used to derive Console resource names."
  type        = string
  default     = "automq"
...
variable "network_id" {
  description = "Canonical ID of the existing VPC used by the Console firewall."
  type        = string

  validation {
    condition     = can(regex("^projects/[^/]+/global/networks/[^/]+$", var.network_id))
    error_message = "network_id must use projects/<project>/global/networks/<name>."
  }
}

variable "management_subnet_id" {
  description = "Canonical ID of the existing subnet used by the Console VM."
  type        = string

  validation {
    condition     = can(regex("^projects/[^/]+/regions/[^/]+/subnetworks/[^/]+$", var.management_subnet_id))
    error_message = "management_subnet_id must use projects/<project>/regions/<region>/subnetworks/<name>."
  }
}
