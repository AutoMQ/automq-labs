...
- An AutoMQ Cloud environment registered for AWS. The environment installation
  wizard supplies the Environment ID, complete Base64 `CONFIG`, and Console
  container image used by this example.
...
## Get the AutoMQ Environment Metadata
...
1. Sign in to [AutoMQ Cloud](https://account.automq.cloud), click
   **Create Environment**, and select AWS and the deployment Region.
2. Open the environment installation wizard and keep the generated EC2
   installation command or deployment metadata. It contains the complete
   Base64 `CONFIG` value and the AutoMQ BYOC Console image reference.
3. Copy the entire value after `CONFIG=` into `automq_config`. Do not decode,
   edit, or paste only the JSON inside it.
4. Use the image reference from the same wizard as `console_image` when it is
   newer than this example's default.

The Environment ID is shown by AutoMQ Cloud and is also stored as
`environmentId` inside `CONFIG`. Terraform decodes it and exposes it as the
`environment_id` output, so it is not a separate Console-stage input. The
Base64 value also contains the Region, AutoMQ Cloud `clientId`/`clientSecret`,
and ops bucket metadata required by the Console. Treat the entire value as a
...
`console_initial_access_key` and `console_initial_secret_key` are generated by
this Terraform example and injected into the local Console as its initial API
credentials. The Cluster root passes them to the AutoMQ Terraform Provider so
it can authenticate to `console_endpoint` and create the AutoMQ Instance. They
are not AWS access keys and are different from the AutoMQ Cloud
`clientId`/`clientSecret` embedded in `CONFIG`.
